...
class Clipboard {
  ...
  listen() {
    ...
    merge(fromEvent(domNode, 'copy'), fromEvent(domNode, 'cut')).subscribe(
      copyCutHandler
    );
  ...
  cutHandler() {
    ...
    if (startOutBlock == null || endOutBlock == null) return;
      ...
      if (
        ...
        outBlock.forEach((item, index) => {
          if (position === 'start' && index === offset) {
            ...
            if (item.isOnlyChild()) outBlock.remove();
            else item.remove();
          }
        });
      } else {
  ...
  async pasteHandler(event: ClipboardEvent): Promise<void> {
    ...
    if (/html|text/.test(copyType)) {
      ...
      if (
        ...
        if (offset != null) cursorBlock?.setCursor(offset, offset, true);
      } else {
