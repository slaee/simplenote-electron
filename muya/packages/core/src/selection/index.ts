...
class Selection {
  ...
  get type() {
    ...
    if (!anchorBlock && !focusBlock) return 'None';
  ...
  private listenSelectActions() {
    ...
    const handleMousemoveOrClick = (event: Event) => {
      ...
      if (type === 'mousemove' && !this.selectInfo.isSelect) return;
      ...
      if (type === 'mousemove') this.selectInfo.selection = newSelection;
      else this.setSelection(newSelection);
    };
